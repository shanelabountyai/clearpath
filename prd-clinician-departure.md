...
| D-24 | The plan screen's client names ride on `departure.read`, never `client.read` | Settled in Phase 4. Admin's `client.read` is break-glass, so resolving names through the client resource would have been a practice manager breaking glass to do routine work — D-21's wall, met again from the read side. P0-3 already gave this cell a client list at the demographic tier, the tier the calendar shows the manager through `appointment.read`. One read, one audit row, names and codes and nothing else, asserted by key set |
| D-25 | The unread-alert blocker is rendered as a count, never as a client | Front desk reads plans, and "this client has an unread risk alert" is a clinical signal about a named person that hard rule 9 routes to one clinician. The fix is the one person entitled to it reading it before they go, which needs no client named on a shared screen |
| D-26 | A decision is refused at the door, and execution moves only who is still on the caseload | Phase 3's `executeDeparture` repointed whatever assignment it was handed. A client front desk gave to somebody else during the thirty days would have been taken back from their new clinician on the last day. `decideAssignment` refuses a client off the caseload or a receiver the matrix would never let write a note, and execution and the blocker scan both read assignments through the live caseload |
...
| D-27 | P1-3's message names no clinician; the name stays behind the client's portal link | Settled in Phase 5, and a departure from P1-3's own example. No client message names a clinician, because a full name is one search away from what kind of practice this is and the deny-list cannot catch a name — which is the lock-screen leak the messaging name exists to prevent. The portal already shows who each session is with. The body gives the first moved session's date and says the day and time stay the same. Queued inside the transaction; nothing for `none` (not even a minted link), for a client with nothing booked, or for a discharge, whose ending is a conversation |
| D-28 | The departure work-list is a `/worklists` section of counts, not a second readiness screen | The plan screen already answers "is this ready?" and is the one place clients are named. `departureWorklist` runs the same `blockersOf`, counts by kind, and returns no client id — D-25 applied to the whole list. Unsigned notes sit beside the blockers, not among them: they never stop execution, they become holes |
| D-29 | The purge preview and the abandoned-note count are per departure, on `departure.read`, and never name a client | No cell lets anybody but the author near a process note, and admin's `progress_note.read` is break-glass. Both surfaces read a departure's consequences, not the notes. The preview shares the sweep's window and author-keyed query, so they cannot disagree about what is due |
| D-30 | Execution is refused before the last day | Found by the seeded demo (WRITEUP §34): `executeDeparture` moves sessions from the last day on and closes the account whenever it runs, so an early click left every session between then and the last day on a deactivated clinician. That is D-06's silent half-moved state, reached with no clash at all. Refusing keeps the preview scan and the execution on one window (D-22), and it is what D-09's two moments already meant. The cost is accepted: somebody leaving early is a withdrawal and a new notice, which throws away the decisions made so far. Moving from the earlier of now and the last day was the alternative, and it would have made the scan flag clashes in weeks that only move if somebody clicks early |
| D-31 | The unread-alert blocker asks routing where each alert would land after the departure, and execution reroutes them in one pass after its writes | Settled 2026-09-12, from §36's own "what it does not do". P0-7 moved alerts inside the disposition loop, one assignment at a time, so an alert the leaver held about somebody *else's* client — a departed supervisee's, handed here by P0-7 at that supervisee's departure (leave D-26) — was never seen, and the account closed holding it three lines after the supervision repoint had named its new owner. Execution now reroutes every unread alert still addressed to the leaver once the caseload has moved, the supervisees have repointed and the account has closed, so `routesOf` reads the practice as it stands instead of a per-client projection of it — and the projection was the bug, because it could only be built for a client the loop knew about. The scan is the same question thirty days early, where a projection is unavoidable: `afterDeparture` applies the three moves to one client's routing facts and `strands` asks whether the answer is nobody — the leaver, or a treating clinician who has departed with no supervisor above them. Wider than the `leaver has no supervisor` test it replaces, and resolvable where that one was not: naming a receiving supervisor clears an inherited alert the old scan blocked with nothing on the screen that could clear it. Rejected: leaving it to the hourly leave sweep, which only reroutes what a leave touches |
...
- **"An alert can still strand on a departing coverer."** True. When the leaver holds an alert only because they cover somebody else's leave, routing names them and the scan blocks the departure rather than moving it (D-31). The fix is on that leave — name another cover, or end it — which is `leave_open`'s reasoning applied to a leave that is not the leaver's. It stays a refusal because choosing who covers a clinician is a decision somebody makes, not a route software picks. The scan routes on the last day, not the day it is read, so a leave that is over by then does not block, and the leave's own screen counts a cover as unavailable when their last day falls on or before the leave's (WRITEUP §44).
...
- ~~**Phase 4:**~~ **Landed.** The plan screen with the blocker list, `decideAssignment` and `setReceivingSupervisor` under `departure.update`, P0-4a's drafts list for the leaver, the `processNoteAfterDepartureDays` field, and the departing marker in the person picker. Names on `departure.read` (D-24), the alert blocker as a count (D-25), decisions refused at the door and execution through the live caseload (D-26); WRITEUP §32.
- ~~**Phase 5:**~~ **Landed.** P1-1 as a `/worklists` section of counts linking to the plan (D-28), P1-2 on `getClient`, P1-3 queued inside the transaction with the name behind the client's portal link (D-27), P1-4 on the practice report, P1-5 on `/practice` (D-29); WRITEUP §33.
- ~~**Capstone demo:**~~ **Landed** as the seeded departure and `departure-demo.spec.ts`, to the Lagging numbers rather than the sketch below (which predates them): fifteen clients, twelve transferred, the clash fixed by changing a receiver, the sweep on an advanced clock. WRITEUP §34. The original sketch: notice is given and the clinician disappears from new-caller assignment the same afternoon; the plan screen shows one Tuesday-evening clash and refuses to execute; the clash is moved; execution transfers eleven clients, discharges two, refers one out, abandons a draft, and repoints an unacknowledged alert — all on one transaction. Then the receiving clinician opens her new client's record and reads four years of Alex's progress notes; then she opens the process-note panel and is refused, and so is the practice manager, and so is the practice manager with break-glass. Then the clock advances past the window and those notes are not there for anybody to be refused.
