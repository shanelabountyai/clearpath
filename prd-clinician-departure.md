...
- **(Builder)** What is the new read rule actually called? `recordReader` names a role rather than a relationship, and every other rule in the file names a relationship (`author`, `treating`, `recipient`, `self`). The honest options are a clumsy enumeration (`authorSupervisorOrTreating`) or a short name carrying a comment. Prefer the clumsy one if it reads at the call site — this file's convention is that a rule name says exactly who, and clarity beats elegance in the one file that is the whole policy.
...
- **(Builder)** Does the process-note sweep run on the existing scheduled path with the inquiry purge and the confirmation cadence, or its own? Prefer the existing one — a second scheduler is a second thing to forget, and intake's build notes already made this call.
...
- **Phase 2:** P0-1 (schema), P0-4b (`abandoned` and its transition), P0-10 (`unreachableSince`, the trigger, the sweep), P0-11 (audit). The trigger is asserted against a real database or it is not asserted.
- **Phase 3:** P0-6 (the transaction and the rollback assertion), P0-7 (alerts), P0-8 (supervision and the `RESTRICT` change), P0-9 (the two moments).
