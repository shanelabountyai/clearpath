...
- **A real carrier.** The delivery port and its receipts shipped in P2; the only driver is an offline simulation. Nothing leaves the machine, and the README's Scope Honesty banner says so.
...
- ~~Per-client cadence selection~~ **— shipped 2026-09-05.** `full` / `day_before` / `day_of` on the client record, with a stated choice beating the earned streak cap and no value meaning "no messages" — that stays `reminderPreference: 'none'`, which is a safety setting and the only one that ends the fee. Shipping it pushed the charge rate to 5.03%, past the over-firing line below, because a day-of client's only message was arriving with an hour to spare; the fix was a third precondition, `answerable`, checking there was time to **answer** and not just time to ask. Rate is now 4.57%. See `src/scheduling/confirmation.ts`, `src/messaging/carrier.ts` and WRITEUP §14.
...
3. ~~**Charging on "queued" is dishonest while nothing sends.**~~ **Closed 2026-09-05.** The fee's precondition is now a delivery receipt: `deliveryProven` in `carrier.ts`, asked by the sweep before anything about money, with a grep lint refusing any future path to `no_response` that does not consult it. A session whose reminders never arrived is exempted to `not_required` — never `no_response`, because the client did not do anything — under its own audit code, and the client lands on a front-desk work list so the exemption produces a phone call rather than silence. Measured on the seeded quarter: 33 fees from 685 eligible (4.82%), against 34 from 692 (4.91%) when a queued row was enough, with 7 sessions asked about and never reached and none of them charged. What remains open is that the carrier is simulated, which is Scope Honesty rather than this risk.
