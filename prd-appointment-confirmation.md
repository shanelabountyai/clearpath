...
- **Per-client cadence tuning.** One practice-wide cadence in v1. Per-client stage selection is P2 and mostly a settings-UI problem. *(shipped 2026-09-07 as P2-3 — it was mostly a settings-UI problem, and the part that was not is that a selection must not stack with the streak cap)*
...
- ~~Per-client cadence selection (a client who wants only the day-of nudge).~~ **Shipped 2026-09-07 (P2-3).** `Client.reminderStages` is an empty-by-default list of the same three stages the rule already speaks in: empty means the practice cadence, streak cap and all, and a non-empty list is the client's own answer. It **wins over the cap rather than intersecting with it** — intersecting a chosen `d0` with a capped `d1` is the empty set, so a client who asked for *fewer* messages would be silenced entirely, four confirmations after somebody ticked the box rather than on the day. It narrows and never promotes: `dueStages` applies every rule it applied before, `reminderPreference: 'none'` still outranks it, and where it queues something the promotion to `pending`, the outbox row and the delivery receipt the fee rests on are unchanged.
...
- ~~Confirmation state feeding the waitlist: a decline at `d5` is a five-day-notice opening, which is exactly what a waitlisted client can take.~~ **Shipped 2026-09-07 (P2-2).** `waitlistOpenings` joins the two: every appointment in the next 30 days that is free (`cancelled`, `late_cancelled`) or about to be (`scheduled` + `declined`), each carrying the waitlist entries whose weekday and time window fit that hour. The two kinds are labelled apart rather than merged — a declined hour is still on the books until somebody rings the client, because an inbound keyword may record an answer and may never move a session. Nothing is booked, offered or messaged; it is a list of phone calls, and a client is never offered the hour they just gave back.
- ~~Multi-language message bodies (the deny-list is English-only and would need one per language — a real gap, named).~~ **Shipped 2026-09-07 (P2-4).** `Client.language` selects the bodies *and* the deny-list, as one thing: `Record<Language, ...>` over both means a new language does not compile until it answers for all six templates and does not pass its tests until `DENY_LISTS` answers for it too. The gate reads **every** language's list against every body rather than the client's own — bodies are routinely a mix, the language a message is *read* in is not a fact this system holds, and checking all of them means adding a language can never weaken the gate for the ones already shipping. The bug underneath both halves was three characters: `'Depresión'.toLowerCase()` does not contain `'depresion'`, and the inbound classifier's `[^a-z]` strip turned `sí` into `s` — one `fold()` now serves both. The inbound phrase table is the union of every language's, read language-blind because an inbound message does not arrive with a language on it, with a test asserting no phrase means opposite things in two languages. Out of scope and named: the portal itself is not translated, and there is no language detection anywhere.
