...
- A real carrier integration behind the outbox, with delivery receipts — which would let the fee rule require *delivered*, not merely *queued*, and is the single biggest honesty upgrade available to this feature.
...
3. **Charging on "queued" is dishonest while nothing sends.** Today the fee's precondition is an `OutboxMessage` row, which proves the practice *intended* to ask. When a carrier is attached, the precondition must become a delivery receipt, or the practice charges for its own failed sends. Named in P2; a real deployment cannot skip it.
