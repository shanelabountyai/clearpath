...
- [x] A public inquiry form that writes an `Inquiry` directly — `/enquire`, bilingual, structured fields only. A `public` role holding one matrix cell (`inquiry: create`, rule `unconditional`); a per-submitter hourly ceiling keyed by HMAC of the address, never the address; a honeypot answered with the same screen a person gets; and `publicInquiryEnabled` as a kill switch that is off by default. No free-text field, deliberately — see D-07.
- [x] Inquiry assignment to a clinician's own queue, with capacity signalling — `Inquiry.assignedClinicianId`, distinct from `requestedClinicianId`; assignment is `update` on `inquiry`, a cell front desk and admin already hold and clinicians do not. Capacity is a new resource with a new rule: `capacity: { read: 'always', update: 'self' }` for clinicians, `read` only for front desk **and for the practice manager**, who is the one role denied a non-clinical cell on this page. The declared half is `User.acceptingNewClients`; the measured half — caseload and queue depth — is counted off rows that already exist and has no column. A closed clinician is never refused an assignment — see D-09, D-10.
- [x] Referral-source detail for `gp` and `referred_out` — which practice, which doctor — `Referrer`, one table serving both directions, with `Inquiry.referrerId` and `Inquiry.referredOutToId`. A new resource `referrer` shaped exactly like `inquiry` for clinicians (`read create`) and held in full by front desk and the practice manager; the **`public` role holds nothing here**, so an anonymous "my GP sent me" stays a bare code and somebody rings back. Which code the entity belongs with is a database CHECK on each column, not a validator. Entries are retired, never deleted — see D-11, D-12.
...
- **D-07: The public form has no free-text field, and the phone call still does.** The PRD rejected structured-only fields as unusable "for a person on a phone", and that reasoning does not reach a web form, where nobody is on a phone. `Inquiry.note` is already named below as the honest weak point: front desk hears "I've been having a hard time since my brother died" and types "prefers mornings", and that human filter is the mitigation. A textarea on a public counselling form is the same field with the filter removed, and it would receive clinical disclosures as a matter of course rather than occasionally. So the form collects a name, one way to make contact, an optional clinician and a referral code, prints a line asking the reader not to write about their health, and leaves the rest to somebody on a phone. What it costs: a submitter who can only do evenings has nowhere to say so.
- **D-08: `public` is a role in the matrix, not a guard bypass.** An anonymous write could have been a route that skipped `guarded` — fewer moving parts, one fewer role, and no change to the permission file. It fails the test this project exists to pass: "what can an unauthenticated request do to this database" has to be answerable by reading one file. As a bypass it is answerable only by auditing every route, for ever. Its rule is named `unconditional` rather than reusing `always`, because `always` means "any actor in this role" and every other role holding it was hired.
- **D-09: Capacity is its own resource, and the practice manager cannot set it.** The obvious home for "am I taking new clients" is a column on `User` behind `user: { update: … }`, which admin already holds. That cell is roles and supervision, and handing a clinician the reach to set their own capacity through it would hand them the reach to set their own role — the argument P0-4 already makes for `discard` over a generic `delete`. So `capacity` is its own resource holding one boolean about oneself, and the interesting cell is the denial: admin reads every row and writes none. A practice manager who can mark a clinician open has turned the signal from "what this clinician can carry" into "what the practice would like", and the row it overwrites was the only record that somebody disagreed. Removing a departing clinician is `user.update`, which admin does hold; declaring that somebody has room is a different act. What it costs: a clinician on leave with their books left open is a wrong signal nobody else can correct, and the practice fixes it by asking them.
- **D-11: One table for both directions, and the internet cannot write to it.** `gp` and `referred_out` are the same six surgeries seen from two sides — the practice you refer an eating-disorder case out to is the one that sends you their anxiety referrals next month — so `Referrer` serves both and two tables would have held that relationship twice and let the halves drift. The cell worth reviewing is the empty one: `public` holds nothing on `referrer`. The anonymous form may leave an enquiry; it may not append a string to a directory the whole practice reads and the report aggregates by, which is a write with a far longer life than the row it came in on. What it costs: a public submission that says "my GP sent me" is a code with no detail until somebody rings back — and that is the same call the form was always going to need.
- **D-12: Which code the entity belongs with is a CHECK, not a validator.** `referrerId` is refused on any source but `gp`, and `referredOutToId` on any reason but `referred_out`, by `inquiry_referrer_only_for_gp` and `inquiry_referred_out_has_a_reason` in the database — the same argument hard rule 5 makes about append-only audit rows. The service *shapes* (it drops a surgery when the source is not `gp`) rather than refusing, because the form has no way to hide a picker without JavaScript and a `Conflict` there would be a 500 on an ordinary change of mind; the database is what makes the disagreeing row unrepresentable for every writer that never passes through the service. Two mechanisms, one of them unbypassable. And nothing here is ever deleted: a surgery that closed its list in June is a fact about June, not a licence to rewrite what a report said about March, so retiring is an `update` and `onDelete: Restrict` says the same thing in the schema.
- **D-10: The signal is never a gate, and half of it is never typed.** Assigning a call to a clinician who has closed their books succeeds, with the warning drawn on the row rather than in place of the control. A hard block would be trivially correct and operationally wrong: a caller who rang and asked for Alex by name still belongs with Alex, and a block would only teach people to flip the boolean to get past it. `no_capacity` was already in the discard vocabulary and is the honest ending when the answer is really no. The measured half — active caseload, and calls already waiting in that queue — is counted from rows that exist rather than kept in a column, because a number somebody has to remember to update is wrong by Thursday, and front desk would believe it.
...
- **"You have put an unauthenticated write endpoint in front of a clinical database."** One table, one status, one role holding one cell, and four things in front of it: a kill switch that is off until a practice turns it on, an hourly ceiling per submitter, a honeypot, and a field set with nowhere to put a sentence. What it can produce is an `Inquiry` row at `open` with a name and a phone number — the same row the front desk produces by answering the phone, and the same purge counting down on it. The residual risk is a flood of plausible fake enquiries wasting front-desk callbacks; the ceiling bounds the rate, and the kill switch bounds the incident.
