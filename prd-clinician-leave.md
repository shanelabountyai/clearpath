...
**Status:** v0.2 — **reviewed 2026-09-10** (D-14 to D-17 added); Phases 1–4 built, capstone seeded (D-19 added); P1-1 and P1-5 built (D-20 added); P1-3 reviewed and built (D-21 to D-24 added); P1-4 built (D-25 added); a departure while the supervisor is away routed (D-26 added); P1-4 extended to the returning supervisor, with a dismissal (D-27 added). Decided 2026-09-10: PRD first, then build in phases. Feature PRD, child of `prd-clearpath-counseling-ops.md`; the P2 that `prd-clinician-departure.md` deferred by name
...
- **P1-4: "While you were away."** Nour's first screen back lists flagged submissions, sessions Dev held and notes Dev wrote during the window. All of it comes through reads Nour already holds as treating clinician. No new cell. **Built 2026-09-11 (D-25):** a "While you were away" section at the top of `/worklists`, where Home sends the person back for a fortnight after the last day; three treating reads in one transaction, and never a process note. **Extended 2026-09-14 (D-27):** a supervisor back gets a fourth list — the countersignatures their supervision cover gave — on the `authorSupervisorId` door supervision already reads notes through, and asked for only when they have supervisees; and anybody back can say they have read it, reversing D-25's "nothing is written on return".
...
| D-27 | A supervisor back reads their cover's countersignatures on a fourth door, and D-25's "nothing is written on return" is reversed: `Leave.backDismissedAt` | Settled 2026-09-14 extending P1-4. The supervision list is not about the caseload, so it cannot ride `{ clinicianId: actor.id }`; it takes `{ authorSupervisorId: actor.id }`, the cell the co-sign queue already reads supervisee notes through, and is asked for at all only when the person has supervisees — so a therapist back still makes exactly three audit rows and a supervisor makes four. D-25 argued a dismissal was a write and a stored state for a list that expires anyway; a fortnight of a banner nobody can clear reads as an app that cannot be told something is dealt with, and the fortnight stays as the backstop for the person who never says so. The write is self-scoped by the query (`userId: actor.id`), not by a new cell: a clinician holds no `leave.update`, and widening one to clear a banner would hand them the cell that decides who covers their clients. It writes no clinical fact and reveals none, so it is not on the record — the summary it clears is |
...
- **P1-4:** landed 2026-09-11. `whileYouWereAway` on `/worklists`, and Home lands a returning clinician there for 14 days (D-25). Extended 2026-09-14 with the supervision list and a dismissal (D-27).
