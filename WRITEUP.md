...
| SEC-01: one shared password over the whole site, in `proxy.ts`, and a production build with none set answers 503 (2026-09-23) | The identity picker lets anyone who reaches the site become anyone, so the site itself has to be closed. A gate that is off when its variable is missing fails open exactly when somebody forgot it. Only `/api/cron/*` is exempt, because each of those routes checks its own bearer and Vercel Cron cannot send Basic. The file is `proxy.ts`, not `middleware.ts`: Next 16 renamed the convention |
| SEC-01: the e2e sweep sends a throwaway password rather than opting out (2026-09-23) | The sweep is a production build, which is the case the gate fails closed in. An opt-out flag is a switch that can be set in production; a credential in `playwright.config.ts` cannot. It also means the sweep tests the shipped gate |
| SEC-02: `/enquire` says in both languages that it is a demo with invented people (2026-09-23). No nightly reseed | Behind the gate, only people who were given the password can read what is typed. The production seed takes about 25 minutes, which no serverless function can run, so a reseed cron would be a second piece of infrastructure to guard a residual risk. Revisit if the password is ever published widely |
| SEC-03: the break-glass cookie is signed over user id and reason; the picker rule is the session rule; the identity cookie stays unsigned (2026-09-23) | A hand-set break-glass cookie reached client records without the audit row `startBreakGlass` writes. Signing it makes that row unavoidable. The identity cookie is not signed because the picker offers every identity anyway; what mattered was that it accepted ids the picker refuses (a client-role user, a deactivated one), and `currentSession` and `switchUser` now share the picker's `where`. Signed state, not a database row, so no migration |
| SEC-04: capability tokens stay plaintext at rest; only the header half is fixed (2026-09-23) | Hashing `PortalLink.token` breaks `ensurePortalLink`, which re-reads the live token each reminder stage to rebuild the `/p` URL — that reuse is D-05's one-revocation-story-per-client. The alternatives (encrypt at rest with a lookup hash; mint a link per reminder run) cost a key and a backfill, or several live links per client, to protect synthetic data behind a demo gate and an expiry. `/f/*` and `/p/*` now send `Cache-Control: no-store` and every page `Referrer-Policy: same-origin` (SEC-05; `no-referrer` sent `Origin: null` on same-origin form POSTs and broke the no-JavaScript enquiry form, fixed 2026-09-23). Revisit if the data stops being synthetic |
| PRD 6 Q5: manual VoiceOver pass skipped (2026-09-23) | Shane's call. Conformance rests on the axe WCAG 2.2 AA sweep (17 pages, in CI) and the derived contrast tests; neither exercises a real screen reader, so announcement order and label wording on the intake form, screener and enquiry form are unverified. Revisit before any real-user use |
