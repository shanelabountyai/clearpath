...
| The role-check lint was a regression test wearing a lint's clothes | It matched `\.role\s*[=!]==` and a role literal on the left of a comparison — the shape the rule had actually been broken in — and nothing else. Planting the same violation four other ways showed all four sail past: `const { role } = actor` then `role === 'therapist'`, aliasing to a local first, `switch (actor.role)`, and `['therapist','supervisor'].includes(actor.role)`. Loose `==` escaped too. Each is how a competent person would naturally write it, so the lint was strongest against the one mistake already caught and weakest against the next one. It now matches by the shape of the comparison rather than one syntax, builds its alternation from `ROLES` so adding a role cannot quietly narrow it, and — the part that makes it trustworthy — carries its own fixtures: five spellings it must flag, and the two places a role name legitimately appears (choosing which users are clinicians, and a deny-list of clinical words that happens to contain `'therapist'`) which it must not. The author-only lint was checked the same way and needed no change: it parses balanced parens and requires `authorId` inside the call, which is shape, not syntax |
| The same sweep over the modules carrying the hard rules, where most survivors were the tool's noise and three were not | 125 mutants over `notes/service.ts`, `guard.ts`, `lifecycle.ts` and `outbox.ts`. The deny-list killed 9 of 9, which is the answer you want from the one module whose job is that a reminder never says why. Of the 51 that survived the whole suite, about 37 are an artefact of the mutation operator rather than a defect: flipping `select: { name: true }` to `false` in a Prisma query only means no assertion names that column, and enumerating every selected field is not a test anyone should write. Three were real. `breakGlassWouldHelp` had every existing test ask it about a door that is shut and none about one already open, so inverting its early return — offering break-glass to someone who does not need it — changed nothing the suite could see. `cancelledAt`, `cancelledById` and `cancelReason` were never asserted absent on a non-cancelling transition, so a completed session could be made to carry a cancellation stamp and read as both in every report. And the attendance summary's four counts were all 1 in the only case that checked them, with `completed` never asserted at all, so a lookup matching the wrong status returned the right number anyway; distinct counts are what make four fields distinguishable. Writing the third of those tests I left an `await` off a promise and it passed on timing — the flake this project keeps refusing, produced while closing a gap about rigour |
| The last eight modules, where the gaps were in what the product tells people | 207 mutants over booking, groups, the client repository, the form schema, forms and portal services, the superbill and trends. `trends.ts` killed 16 of 18 — the module whose argument is that a chart must refuse to draw across a scoring revision holds up — except that `spansVersions` was only ever asserted *true*. Always-true would have satisfied the suite, and always-true means every trend refuses to draw the deltas it exists for; the negative case is now asserted too. Booking asserted the conflict *code* and never the sentence beside it, so 'no therapy room is free' could be shown when the room is free and the clinician is not — the same wrong-reason failure as `b09001c`, one field over. The preferred-room ordering had no test at all: a preference must reorder the candidates without becoming the whole list, or a busy usual room reports the practice full with another standing empty. `setFee` rejected 65.5 and -100 and never tried 0, which is a fee a practice sets on purpose and the guard would have refused. And `validateSubmission`'s date branch had never run in any test, because no fixture has a date field |
| The reporting modules were the thinnest in the project, and the first round of fixes for them was not enough | 96 mutants over the auditor's query, the utilization report, the work lists and the CSV writer killed only 42 — the worst ratio anywhere in this codebase, and these are the P1 features. The utilization test asserted the clinician row as `{ id, completed, telehealth }`, so `noShow`, `lateCancelled` and `cancelled` could each be inverted to count everything that was *not* that status; and room utilization was asserted `toBeGreaterThan(0)`, which holds for any capacity at all — including the midnight-to-midnight one the comment beside it exists to argue against. The waitlist has two bounds and only the upper one had ever decided a match, so a client who said 'nothing before 5pm' could be offered 3pm. Paging was untested end to end. The part worth keeping is that the first six tests written for all this killed 21 of 33 mutants, not 33: the verification pass is not a formality after the fix, it is what showed the fix was partial — a range that stops on a Friday cannot catch an off-by-one in a weekday test, and an assertion on a room that was used says nothing about the rooms that were not. Five more tests, and 29 of 33. Of the four left, one is equivalent (`take + 1` to `take + 2` over-fetches a row that is then sliced off) and three are constants a test could only reach by seeding more than a hundred audit rows |
| A calendar date and an instant needed different readers | `localDateOf` answers which wall-clock day an *instant* falls on in the practice timezone, and it was being used on five `@db.Date` columns. Postgres returns a date column as UTC midnight, and this timezone is behind UTC all year, so every one of those reads came back a day early — never loudly, never seasonally, always by exactly one. A client's date of birth was wrong on their own record. The slot-finder read a vacation range a day early, which is a client booked into a clinician's first day away and refused a slot on the day before it. A series' inclusive `endDate` slipped off the pattern's weekday, so the last session of a fixed-length course was silently never created. `calendarDateOf` reads the column instead, and the lint that keeps it that way builds its field list by parsing `@db.Date` out of the schema, so a date column added next month is covered the day it lands |
| One of the six sites is guarded by the lint alone, and the note says so | The series `startDate` read a day early is behaviourally equivalent for every series this app creates: `anchorOf` shifts forward to the pattern's weekday, and the app always writes a `startDate` that already sits on it, so the shift is absorbed. It stops being absorbed when the start date is off the weekday — the case `anchorOf` exists for — where a Wednesday read as Tuesday anchors a full week early and gives the client a session before their course begins. That is the test. Writing one that "covered" the equivalent case would have been a test that cannot fail |
| The day view's absence banner was correct only on Sundays | It asked `workingWindows` whether a clinician had any time left, against a synthetic base window written as `[{ weekday: 0, startMinute: 0, endMinute: 1440 }]`. `workingWindows` filters the weekly pattern by the date's weekday, so six days a week that base filtered itself to nothing and *any* override — two hours at the dentist — subtracted to an empty day and read as away. Front desk got "Away today: Ada Ling (Dentist)" over a grid still holding her morning and afternoon. `isAway` was sitting in the same module, tested, and used in no production file. The fix is neither: `lostWindows` returns the pattern the overrides take away, which separates the two cases instead of collapsing them, and the banner now says "out 13:00–15:00" where that is the truth. A clinician who does not work Wednesdays produces no absence on a Wednesday, because not working Wednesdays is not news |
| The same rounding-up was in the work list, where it cost phone calls | `vacationImpact` took dates and no minutes, so a two-hour absence handed back every session in the range. A list whose entire value is that each row is a client somebody has to ring filled up with clients nobody had to ring, which is how a work list stops being read. It takes the window now, and the seed carries a part-day absence beside the week-long one so both paths run against real data — the demo's only absence had been a whole week with null minutes, which is exactly why none of this was ever exercised |
| A mutant survived on the query scoping, and the comment says that rather than claiming the fix | Narrowing the overrides and availability queries to the clinicians on screen is data minimisation, not the repair: `absencesOn` walks the clinician list, so an off-screen override could not reach the output either way, and deleting the `user: onScreen` line leaves the suite green. It stays because a therapist reading their own day has no business pulling the whole practice's absence reasons into the request — but a comment that claimed it fixed the leak would be describing a test that does not exist |
| The day view had no tests of its own until this commit | It was covered from above by Playwright, which walks the page and finds the chips it expects, and every bug in this module was invisible to that — the page renders exactly what the module hands it, so the banner was wrong six days a week and the pictures still looked right. Twenty-two tests now cover it directly, and each of the three defects was re-planted afterwards to confirm the suite goes red |
| The break-glass justification is a code, because prose pointed at the audit log cannot be made safe | Every audit row a break-glass session writes carried a sentence an administrator typed into a textarea, and the dialog's own placeholder showed what that sentence looks like: *"client called the practice in distress and their clinician is on leave."* That is a clinical claim about the person the same row identifies by id, in the table that is append-only by database rule and read by the auditor — the one role that may never read a note. Hard rule 3 says ids only. The seed demonstrated the violation rather than avoiding it, and the audit page rendered the column immediately above a footer promising "no names, no note content, no answers" |
| A deny-list would not have fixed it, and that is the interesting part | The outbox has one, and it is the right tool there: a reminder must not say *why* someone attends, and "therapy", "screener" and "crisis" are the words that would. It cannot catch "Jane rang in tears", which discloses both who and why using none of its vocabulary. Free text can be filtered for topic; it cannot be filtered for *a person*. So the channel is closed instead — the portal's reschedule reason code, one tier up. What an auditor needs from this field is the category of claim, and the who, when and which-record are already columns on the row |
| The type is the lint | `Actor['breakGlass'].reason` is now a union of seven codes rather than `string`, so a sentence stops compiling at every site that builds an actor. Turning it on found fourteen: two in the seed and the rest across the suite, among them `'client in crisis, clinician unreachable'`, `'client unreachable, welfare check'` and `'i would like to see'` — each one a fixture that had been writing prose into the audit table on every run. No grep would have been needed and none was written, because the compiler already enumerates the call sites. What the type cannot see is a write that never builds an actor, so the one lint added sweeps `src/` and `app/` for a direct `auditEvent.create` outside the guard, which is hard rule 4's concern as much as this one, and carries its own fixtures |
| The cookie was the trust boundary, and nothing was reading it | The server action checked the reason was at least ten characters. The cookie it set was then copied into the audit row unread, and `httpOnly` stops a script in the browser without stopping a request composed by hand — so a crafted header wrote arbitrary text into an append-only table without meeting even that check. `parseBreakGlass` is where the value becomes typed, and an unrecognised one is not an error page: it is simply no break-glass session, and the ordinary refusal follows. The permission rule re-checks the code rather than trusting whoever built the actor, because that rule is what opens the clinical tier |
| The case reference takes an identifier, not a sentence | Dropping free text entirely would have lost something real: a subpoena has a docket number and an auditor chasing one wants it. The field beside the code is 32 characters with no spaces, so `2026-114` fits and "client rang in tears" does not — not because a list of words rejects it, but because prose is not the shape of an identifier. Writing the parser's tests found it accepting `legal_request:2026-114:extra` by silently discarding the tail, which is a boundary treating something it does not understand as something familiar |
| A second untied `orderBy`, in the query beside the one already fixed | `abac4cd` gave the day view's appointments query a tiebreaker because `startAt` alone is not an order when four rooms are booked at 10:00. The overrides query one line below it still read `orderBy: { fromDate: 'asc' }`, and the absence banner joins those rows' reasons into a sentence — so two blocks bitten out of the same Tuesday produced "Dentist; School run" or "School run; Dentist" depending on physical row order. The test that covered it asserted one of the two and had been passing on insertion order; it now writes them in the opposite order, which is the only version that can fail. This surfaced as a genuine flake mid-run rather than by inspection, which is the argument for the tiebreaker being a rule rather than a fix applied where somebody noticed |
| The last four scheduling modules, swept the same way | 202 mutants over `availability.ts`, `time.ts`, `dayview.ts` and `calendar.ts`. Four gaps were real and none of them was in a branch anybody would have thought to look at. `freeSlots` rounds a window's start *up* to the fifteen-minute grid, and every window in every fixture already sat on a boundary — so rounding down survived, and rounding down offers a nine o'clock to a clinician who is out until five past, which `bookAppointment` would then have taken at face value. `merge` sorts before it walks, and nothing passed it anything unsorted; the query that feeds it names no `orderBy` at all, so without the sort a clinician's morning is absorbed into their afternoon and the day reads as starting at one. `daysBetween` was only ever asked about two dates in one March week, which is exactly the pair a constant month shift inside it cannot disturb — February is what separates them, and `occurrenceDates` divides that number by fourteen to decide which fortnight a biweekly client has. And `layoutTracks` falls through to the id when two sessions tie on both start and end, which is the determinism the appointments query's own tiebreaker exists for one layer up; the test that looked like it covered this covered the *end-time* tiebreak instead |
| `calendar.ts`'s three were all about telling one clinician's day from another's | `absencesOn` filters the weekly pattern down to the person it is answering for, and filters the reason list by that person and by kind. Every fixture in the file had a single clinician with a pattern and a single override with a reason, so taking somebody else's hours, or somebody else's reasons, or the reason attached to an *added* shift, changed nothing any assertion could see. The third of those is a disclosure: "Ada Ling away today (School pickup)" over Bo Ferreira's afternoon. Separately, `where: { active: true }` on the room query had never met a deactivated room, so a closed consulting room keeping its column — a grid inviting the front desk to book into it — was untested |
| `isAway` was dead code that its own test kept alive | The previous commit's note said it sat in the module, tested, and used in no production file. It still did. A test is not a use: what it was doing was making an unused export look maintained, and making one mutant survive that had nothing to defend |
| Most of what survives now is the tool, and saying so is the point | Of the twelve left in `availability.ts`, ten are `?? 0` and `?? 1440` defaults where the operator's alternative differs only for an override that ends at midnight, and boundary constants that only a clinician working from midnight could distinguish. Of the eight in `dayview.ts`, five are sign-preserving edits to a comparator's `-1`/`1`, one is a removed non-null assertion the runtime never sees, and one is an `of: 1` that `flush` overwrites on every path. Writing tests for those would be writing tests for the mutation operator. The one that was worth planting by hand — deleting the id tiebreak outright, which no single-token edit expresses — is the one that found the real gap |
| A mutant of `addDays` hung the sweep instead of failing it | `occurrenceDates` walks `cursor = addDays(cursor, step)` until it passes the window's end. Shift `addDays` back a month and the cursor moves backwards, `cursor <= last` never stops being true, and the run never returns — so the harness sat on one mutant while a suite that could not finish was mistaken for a suite still running. The production function is correct and a guard against an impossible input is not worth adding; the note is about the tool. A mutation run needs a timeout, because "no result" and "no failure" look identical from outside |
| The offered slots were the only thing stopping a booking at one in the morning | The booking form renders `availableSlots` as radio buttons and the server action reads whichever `startMinute` arrives — so the constraint lived in the markup. A composed request produced a real, audit-logged session at any minute of the day, and the day view's grid is a fixed 08:00–19:00 inside a box that computes to `overflow: auto` on both axes, so a session before eight was clipped where nothing could scroll to it. Not hypothetical in shape: it is the same trust boundary as the break-glass cookie two commits ago, one form over |
| The check is against the pattern and not the overrides, which is the whole of it | A session inside a clinician's vacation week must still be bookable. Standing weekly clients against a week off is this domain's cascade — `vacationImpact` exists so that somebody rings each of them — and refusing the booking would convert that conversation into a silent gap, which is the outcome the work list is built to prevent. A session at one in the morning is a different question: no week contains it and nobody is going to ring anybody about it. So the rule is the weekly pattern, overrides deliberately not applied, and the window rather than the fifteen-minute grid, because the grid is how slots are *offered* and not what makes an hour legitimate |
| Enforced at all three doors, because one of three is a convention | `bookAppointment`, `bookGroupSession` and `rescheduleAppointment` each take a start minute and each trusted it. Fixing the one that prompted the question is how a rule becomes a place somebody once noticed a bug |
| Six fixtures were booking outside working hours and nothing had ever objected | Turning the check on found them: a fifty-minute session starting at 17:00 against a clinician who finishes at 17:00, a series moved to Thursdays for a therapist who works Tuesdays, four therapists in the group and portal suites created with `makeUser('therapist')` and no availability row at all — which is a clinician who works no hours, booked all the same. None of these were the subject of the test they were in, which is exactly why they survived: a fixture is the part nobody re-reads |
| The roster page was the one read in the application with no permission check and no audit row | `/groups/[id]` called `await requireSession()` — discarding the session — and then `getGroupSession(id)`, a function that took no actor because nothing had ever needed to pass one. It returned six clients by name, code and attendance status. Every other record page in `app/` passes an actor to a guarded getter; this was the only one that did not, and the auditor — whose entire design is that they see the access event and never the content — could open it by id. It is `guardedAll` now, for the reason booking a group already used it: hard rule 4 wants one audit row per record touched, and six people in a room is six records |
| Three separate gaps had to line up for it to survive, and each of them looked reasonable | The route sweep skipped `[id]` segments, on the grounds that "a `[id]` segment needs a real record, and the record pages are the ones that already got this right". The seed created no group session at all, so there was no id to open it with. And `groups.test.ts` called `getGroupSession(group.id)` and compiled, because a getter with no actor makes refusal inexpressible — you cannot write the test that fails. Any one of the three closed would have caught it |
| The seed had no group session, so nothing the feature is made of had ever met real data | Not the chip that collapses six attendee rows into one hour, not `groupStatus`, not the roster page. `groups.test.ts` covered the module and the e2e sweep covered the calendar, and between them sat a page neither one opened. Three Mondays now carry a skills group — two behind the pinned TODAY with mixed attendance, one ahead that can still be cancelled — and the busiest day the screenshot spec picks is now a day with a group on it, so the README's calendar picture shows the collapse and shows `groupStatus` reading `completed` over a roster containing a no-show |
| Extending the sweep to `[id]` routes catches the missing denial panel, and could never have caught the missing guard | Worth saying precisely, because the tempting sentence is that the sweep would have found this. It would not. With no guard there is no `Forbidden`, so there is no 500, and every route answers 200 — planting the unguarded read back and running `denials.spec.ts` confirms it passes. What the sweep does catch is the class the nine list pages were in, now on dynamic routes too: re-planting a `/groups/[id]` without `withDenial` fails it, naming the role. The missing guard is caught by a unit test asserting the auditor is refused, and the reason nobody had written one is the signature |
| So the lint is on the shape both offenders shared | `await requireSession()` whose answer is assigned to nothing: the call made for the redirect it performs, with the actor thrown away. It is a proxy and the note says so — it cannot judge whether a bound actor is *used* well — but it is exactly what `/groups/[id]` looked like from outside, and it found a second one |
| `/forms` was the second, leaning on the navigation as though a hidden link were a control | The page queried `formTemplate.findMany` directly. `form_template: read` is absent for front desk and for the auditor, and the nav gated the link on `update`, so the only thing keeping either of them out was not being shown the door — the same arrangement as the booking form rendering the offered slots while the action took any minute it was handed. The write was never at risk: `publishTemplate` guards `create`, so a POST from the wrong role was already refused. It was the reading that went unasked |
| The heading counted rows and the grid drew chips | "22 sessions" over seventeen chips, reconcilable only by spotting a `×6` and doing the arithmetic. Both numbers are true and they answer different questions: six people in a skills group are six sessions, six notes and six fees, which is what the practice runs on, and one booking of one room for one hour, which is what is on the screen. The heading says both — and only when they differ, because on a day with no group it would otherwise be the same number twice |
| The module that decides who a request is had no test file at all | The matrix is exhaustively tested about what an actor of a given role may do, and `session.ts` — which turns a cookie into that actor — was tested nowhere, because its decisions were wrapped in a `cookies()` call and a database read. Two of them are worth stating on their own. A deactivated user is nobody: the cookie in somebody's browser outlives the decision to remove them, and this is the only place that decision reaches an existing session. And an unrecognised break-glass cookie is simply not a break-glass session rather than an error — the cookie is the trust boundary, `httpOnly` says nothing about a request composed by hand, and the answer to a hand-written one is the ordinary refusal. `sessionFor` now takes the user row and the cookie value as arguments so both can be asserted without a request; `currentSession` is the shell that supplies them, and a real deployment still replaces this one file and nothing else |
