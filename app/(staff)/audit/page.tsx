    ...
    <>
      <PageHeader
        ...
        actions={
          <a
            ...
            className="rounded-[var(--radius)] border px-3 py-1.5 text-[12.5px] font-medium"
      ...
      <Card className="mb-4">
        <form className="flex flex-wrap items-end gap-3">
          <div>
            <label htmlFor="clientId" className="block text-[11.5px] font-medium tracking-wide text-subtle uppercase">Client</label>
            <select id="clientId" name="clientId" defaultValue={q.clientId ?? ''} className="mt-1 rounded-[var(--radius)] border px-2 py-1.5 text-[13px]" style={{ borderColor: 'var(--border)', background: 'var(--surface-raised)' }}>
              <option value="">Any</option>
              {clients.map((c) => <option key={c.id} value={c.id}>{c.code}</option>)}
            </select>
          </div>
          <div>
            <label htmlFor="actorId" className="block text-[11.5px] font-medium tracking-wide text-subtle uppercase">Person</label>
            <select id="actorId" name="actorId" defaultValue={q.actorId ?? ''} className="mt-1 rounded-[var(--radius)] border px-2 py-1.5 text-[13px]" style={{ borderColor: 'var(--border)', background: 'var(--surface-raised)' }}>
              <option value="">Anyone</option>
              {users.map((u) => <option key={u.id} value={u.id}>{u.name}</option>)}
            </select>
          </div>
          <div>
            <label htmlFor="resource" className="block text-[11.5px] font-medium tracking-wide text-subtle uppercase">Resource</label>
            <select id="resource" name="resource" defaultValue={q.resource ?? ''} className="mt-1 rounded-[var(--radius)] border px-2 py-1.5 text-[13px]" style={{ borderColor: 'var(--border)', background: 'var(--surface-raised)' }}>
              <option value="">Any</option>
